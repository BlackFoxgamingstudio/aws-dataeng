# Sovereign Aws Dataeng (`sovereign-aws-dataeng`)
...
[![CI](https://github.com/BlackFoxgamingstudio/aws-dataeng/actions/workflows/ci.yml/badge.svg)](https://github.com/BlackFoxgamingstudio/aws-dataeng/actions/workflows/ci.yml)
[![n8n Integration](https://img.shields.io/badge/n8n-workflow_ready-orange.svg)](n8n/workflow.json)
[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](LICENSE)

> **Enterprise Standalone Package**: AWS enterprise data lake and ETL pipeline SDK. Manages AWS Glue job definitions, Athena schema catalogs, S3 partitioned data lakes, and EMR Spark jobs for big data analytics without vendor lock-in.
...
| **REST Gateway** | ✅ Live on Port `8801` | `POST /api/v1/execute`, `GET /health` |
...
`sovereign-aws-dataeng` is an independently packaged, zero-dependency software library and microservice engineered as part of Russell Alan Powers' 10-year software engineering portfolio.

It delivers robust capabilities in **Cloud Data Engineering & ETL** and provides seamless integration with n8n event workflows.

```
┌───────────────────────────┐         HTTP POST          ┌───────────────────────────────────────────┐
│        n8n Engine         │ ─────────────────────────> │        sovereign-aws-dataeng Adapter        │
│   (Port 5678 Webhook)     │ <───────────────────────── │             (Port 8801)                 │
└───────────────────────────┘       Idempotent JSON      └───────────────────────────────────────────┘
                                                                               │
                                                                               ▼
                                                         ┌───────────────────────────────────────────┐
...
- **Primary Module**: `from sovereign_aws_dataeng import GlueJobRunner, AthenaQueryClient, S3PartitionManager, EMRSparkSubmitter`
...
git clone https://github.com/russellpowers/sovereign-aws-dataeng.git
cd aws-dataeng
...
sovereign-aws-dataeng --exec process_data --payload '{"sample_key": "sample_value"}'
```
...
- **Microservice Port**: `http://localhost:8801`
- **Inbound Trigger Route**: `POST /api/v1/execute`
- **Integration Workflow**: `Daily batch cron -> Trigger AWS Glue ETL crawl -> Ingest S3 parquet -> Notify n8n data warehouse pipeline`
...
4. Start the background webhook adapter:
   ...
   ```
5. Dispatch your test event to `http://localhost:5678/webhook/aws-dataeng-trigger`.
