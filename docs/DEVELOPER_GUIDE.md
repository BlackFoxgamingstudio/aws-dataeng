# Developer Guide: Sovereign AWS DataEng
...
```bash
# 1. Clone
git clone https://github.com/BlackFoxgamingstudio/aws-dataeng.git
cd aws-dataeng
...
export AWS_DEFAULT_REGION=your_value_here
export SBB_S3_BUCKET=your_value_here
...
│   ├── cli.py               # Typer CLI (sovereign-aws-dataeng)
...
```bash
# Build
docker build -t sovereign-aws-dataeng:dev .

# Run with env file
docker run --env-file .env -p 8801:8801 sovereign-aws-dataeng:dev

# Full stack (with n8n network)
docker-compose up --build

# Logs
docker-compose logs -f sovereign-aws-dataeng
...
3. Use **SovereignTools** node → configure endpoint to `http://localhost:8801`
...
| Issue | Diagnosis | Fix |
|-------|-----------|-----|
| Port `8801` in use | `lsof -i :8801` | Kill PID or change `SERVICE_PORT` |
